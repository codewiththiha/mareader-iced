...
//!
//! Every page is a stack of independently rasterized layers (the page frame
//! with its blend/filter, the texture, the backdrop), and the compositor
//! snaps each layer's paint rect to the DEVICE pixel grid. Page geometry,
//! though, is `intrinsic_size × zoom_scale` — almost never a whole number of
//! device pixels once the scale factor is fractional (125% / 150% / 175%
//! display scaling). When two adjacent rects round in
...
//! value that becomes a page's drawn size or position goes through [`snap_px`]
...
/// Round `v` (logical px) to the nearest whole device pixel for a display
/// whose device-pixel ratio is `dpr`. Split from [`snap_px`] so the arithmetic
/// is testable on its own: the live half adds nothing but the ratio.
...
/// The display's current device-pixel ratio, fed by the app from its window's
/// live scale factor ([`set_device_pixel_ratio`]) and defaulting to 1.0 until
/// the first report. Stored in thousandths so the read stays allocation-free
/// and lock-free; `snap_to` above stays pure, so the suite proves the rule
/// without a display.
static DEVICE_PIXEL_RATIO_MILLIS: AtomicU64 = AtomicU64::new(1_000);

/// Feed the display's device-pixel ratio (a window scale factor) to the grid.
/// Called on launch and whenever the window rescales — a non-finite or
/// non-positive value falls back to 1.0 rather than poisoning the grid.
pub fn set_device_pixel_ratio(dpr: f64) {
    let millis = if dpr.is_finite() && dpr > 0.0 {
        (dpr * 1000.0).round() as u64
    } else {
        1_000
    };
    DEVICE_PIXEL_RATIO_MILLIS.store(millis, Ordering::Relaxed);
}

fn device_pixel_ratio() -> f64 {
    DEVICE_PIXEL_RATIO_MILLIS.load(Ordering::Relaxed) as f64 / 1000.0
}

/// Snap a logical-px length or offset to the device-pixel grid.
...
mod tests {
    ...
    fn the_boundary_helper_is_the_ratio_the_display_reports() {
        // With no report yet the ratio is 1, so snapping must be the identity —
        // the test run below is only meaningful if it is.
